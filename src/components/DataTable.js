    ...
    <>
      ...
      <Pagination className="float-right">
        ...
        {Object.keys(dataObj).map((d) => (
          <Pagination.Item onClick={() => setPageNumber(d / 1)} active={pageNumber == d}>
            {d / 1}
          </Pagination.Item>
        ))}
        <Pagination.Next
          disabled={pageNumber === Math.round(data.length / pageSize)}
            ...
            return (
              <div className="table-card-item">
                ...
                <span className="text-muted">
                  {dataProcess[head.key] ? dataProcess[head.key](item[head.key]) : item[head.key]}
                </span>
              </div>
            );
          })}
